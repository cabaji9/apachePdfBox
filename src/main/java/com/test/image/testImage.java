...
public class testImage {
    ...
    public static void main(String args[]) throws Exception
    {
        ...
        PDDocument document = PDDocument.load(new File("/Users/User/Downloads/PagareOrdenTasaFijaD201708020901086465.pdf"));
        PDFRenderer renderer = new PDFRenderer(document);

        int numberOfPages =document.getNumberOfPages();
        float resolution = 100f;

        List<Image> images =   obtainAllImagePages(numberOfPages,renderer,resolution);
    ...
    private static List<Image> obtainAllImagePages(int numberOfPages,PDFRenderer pdfRenderer,float dpi) throws Exception{
        List<Image> images = new ArrayList<>();
        for(int n=0;n<numberOfPages;n++){
            BufferedImage image = pdfRenderer.renderImageWithDPI(n,dpi);
            images.add(image);
        }
        return images;
    }
